...
watch(() => props.building, (b) => { collapsed.value = false; copied.value = false; visible.value = !!b })
</script>

<template>
  <div v-if="building && visible" class="building-detail" :class="{ 'building-detail--collapsed': collapsed }">
    <button class="building-detail-toggle" @click="collapsed = !collapsed">{{ collapsed ? '◀' : '▶' }}</button>

    <div class="building-detail-panel">
      <button class="building-detail-close" @click="handleClose">&times;</button>
